...
import { FiPlus, FiSearch, FiMoreVertical, FiUsers, FiGrid, FiSettings, FiBriefcase, FiChevronDown, FiBell, FiCheck, FiX } from "react-icons/fi";
...
const Homepage = () => {
  ...
  useEffect(() => {
    ...
    axios.get("/project/invitations")
      .then((res) => { if (mounted) setInvitations(res.data.invitations || []); })
      .catch((err) => console.error(err));
  ...
  const handleAcceptInvite = async (invId) => {
    try {
      const res = await axios.post(`/project/invitations/${invId}/accept`);
      setProjects((p) => [res.data.project, ...p]);
      setInvitations((inv) => inv.filter((i) => i._id !== invId));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleRejectInvite = async (invId) => {
    try {
      await axios.post(`/project/invitations/${invId}/reject`);
      setInvitations((inv) => inv.filter((i) => i._id !== invId));
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="flex min-h-screen bg-[#F9FAFB] text-slate-900 font-sans">
          ...
          <div className="flex items-center gap-4 ml-4">
            ...
            >
              ...
              {invitations.length > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-red-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full shadow-sm border border-white">
                  {invitations.length}
                </span>
              )}
            </button>
      ...
      {showInvitations && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onClick={() => setShowInvitations(false)} />
          <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 z-10 animate-in fade-in zoom-in duration-200 max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-slate-900">Pending Invitations</h3>
              <button onClick={() => setShowInvitations(false)} className="text-slate-400 hover:text-slate-600"><FiX size={24} /></button>
            </div>
            
            <div className="flex-1 overflow-y-auto space-y-4">
              {invitations.length === 0 ? (
                <div className="text-center text-slate-500 py-8">No pending invitations</div>
              ) : (
                invitations.map((inv) => (
                  <div key={inv._id} className="p-4 border border-slate-200 rounded-xl bg-slate-50 flex flex-col gap-3">
                    <div>
                      <div className="font-semibold text-slate-900">{inv.project?.name}</div>
                      <div className="text-xs text-slate-500">Invited by: {inv.sender?.name || inv.sender?.email}</div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleAcceptInvite(inv._id)} className="flex-1 flex items-center justify-center gap-1 py-2 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 transition-colors">
                        <FiCheck /> Accept
                      </button>
                      <button onClick={() => handleRejectInvite(inv._id)} className="flex-1 flex items-center justify-center gap-1 py-2 bg-slate-200 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-300 transition-colors">
                        <FiX /> Decline
                      </button>
                    </div>
                  </div>
